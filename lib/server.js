		...
		setTimeout(function() {
			...
			if (afterSpecifiedResponse && afterSpecifiedResponse.modifications) {
				...
				while (afterSpecifiedResponse.modifications.length > 0) {
					...
					if (modification.afterResponse && modification.afterResponse.endpoints) {
						modification.afterResponse.endpoints.each(modification.addAfterResponse.bind(modification));
					}
				}
			}
		}, responseDelay);
