...
function Interfake(o) {
	...
	app.all(o.path + '*', function(req, res) {
		...
		if (specifiedResponse.proxy) {
			...
			request[req.method.toLowerCase()]({ url : specifiedResponse.proxy.url || specifiedResponse.proxy, json : true, body : req.body, qs: proxyQueryStrings, headers : specifiedResponse.proxy.headers || null }, function (error, response, body) {
				debug('Response from ', specifiedResponse.proxy, 'is', body, 'with headers', response.headers, 'with error');
				responseBody = body;
