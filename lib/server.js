...
function Interfake(o) {
	...
	function clearRouteForRequest(request) {
		...
		for (i = app.routes[request.method].length - 1; i >= 0; i--) {
			if (app.routes[request.method][i].path === request.url) {
				debug('Clearing existing route at', request.method + ':', request.url, app.routes[request.method][i]);
	...
	function setRouteProperty(request, property, value) {
		...
		for (i = app.routes[request.method].length - 1; i >= 0; i--) {
			if (app.routes[request.method][i].path === request.url) {
				debug('Setting', property, 'for', request.url, 'to', value);
	...
	function fluentInterfaceForMethod(method, parent, top) {
		return function (url) {
			var route = {
				request: {
					url: url,
			...
			return {
				...
				then: {
					get: fluentInterfaceForMethod('get', route, top || route),
					put: fluentInterfaceForMethod('put', route, top || route),
					post: fluentInterfaceForMethod('post', route, top || route),
					delete: fluentInterfaceForMethod('delete', route, top || route)
				}
			};
		};
	}
